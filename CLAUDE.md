...
- **Shares mode** (default): Long/Short toggle. Long stop defaults to LOD, Short to HOD, custom override input. Shares = risk$ / |entry − stop|
- **Options mode**: straddle T-chart — calls left, strike center column, puts right, mirrored columns (Cts, Loss/ct, OI, Vol, Δ, Mid | Strike | reversed). Single-side views (Calls/Puts) use a wider layout with bid/ask/IV/@stop/total
- **Option loss model**: linear delta approx. `premium@stop = max(0, mid + delta × (stop − spot))`, loss/ct = (mid − premium@stop) × 100, contracts = floor(risk$ / loss per ct). Dual stops: call stop defaults LOD, put stop defaults HOD
...
- **5-min chart**: canvas candlesticks (hand-rolled, no chart lib) at the bottom of the page. Walks back up to 6 sessions if today has no data. Dashed HOD/LOD/stop lines. Click chart = set stop. Hover = OHLC crosshair
